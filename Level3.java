...
public class Level3 extends JPanel implements KeyListener,ActionListener
{
	...
	*/
	...
	private String map = "Obstacle-2";

	JFrame f1 = new JFrame();
	
	public Level3(JFrame f1)
	{	
		this.f1 = f1;
		
		addKeyListener(this);		//Adding the keylistener to the const of Level3. Passing'this' because method defined in same class
		setFocusable(true);
...
//************************************************************************************************************************************
	...
	public void paint(Graphics g)		//We can draw things on JPanel using Graphics class. in this method we draw the snake as it moves
	{
		...
		g.fillRect(425, 75, 25, 575);		//(x,y,widht,height)
		
		//Horizontal Box
		g.fillRect(25, 350, 850, 25);
		...
		{			
			...
			snakeLength++;		//length of snake will increase after eating enemy
			
			//Check if player wins 
			if(snakeLength == 518)	//574-56=518
			{			
				repaint();
				gWin();
			}
	...
	void checkEnemyPosition()
	{
		//if the enemy is allocated at a position on obstacle then regenerate enemy position until it is not on the obstacle positoin.
				...
				while((enemyXpos[xPos]>400 && enemyXpos[xPos]<450) && (enemyYpos[yPos]>50 && enemyYpos[yPos]<650))
				{
					xPos = random.nextInt(34);		
					yPos = random.nextInt(23);		
				}
				//Condition for border of top left obstacle
				while((enemyXpos[xPos]>0 && enemyXpos[xPos]<875) && (enemyYpos[yPos]>325 && enemyYpos[yPos]<375))
				{
					xPos = random.nextInt(34);		
					yPos = random.nextInt(23);	
				}
	...
	public void gOver()
	{
		...
		//new UpdateHighScore(highScore,3);
		
		//new MainMenu("clear");		//To clear the previous JFrame of the game
		
		
		GameOverMenu g1 = new GameOverMenu(1,"level3");
		g1.GameOver(score,highScore,3);
	...
	void gWin()
	{
		...
		GameWinMenu g1 = new GameWinMenu(1,"level3");
		g1.GameWin(score,highScore,3);
...
//**********************************************************************************************************************************
	...
	public void actionPerformed(ActionEvent e) 		//This method will be called after timer.start()
	{
		...
		if(right || left || up || down)
		{
			for(int i=snakeLength; i >= 0 ;i--)
			{
				//Condition for vertical of box
				if((snakeXLength[i]>400 && snakeXLength[i]<450) && (snakeYLength[i]>50 && snakeYLength[i]<650))
				{
					gOver();
				}
				
				//Condition for horizontal of box
				if((snakeXLength[i]>0 && snakeXLength[i]<875) && (snakeYLength[i]>325 && snakeYLength[i]<375))
				{
					gOver();
				}
...
//**********************************************************************************************************************************************8
	...
	public void keyPressed(KeyEvent e)			//This method will be called after pressing any key
	{
		...
		if(e.getKeyCode() == KeyEvent.VK_ESCAPE)
		{
			...
			PauseMenu ob = new PauseMenu(1,"level3");	
			ob.PauseMenuF(f1);			
